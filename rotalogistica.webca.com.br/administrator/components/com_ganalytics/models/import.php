...
class GAnalyticsModelImport extends JModel
{
	...
	function getOnlineData() {
		...
		$feed->init();
		if($feed->error()) {
			JError::raiseWarning( 500, JText::_('IMPORT_VIEW_MODEL_FEED_ERROR').' '.$feed->error());
			return array();
		}
	...
	function store()	{
		...
		if (count($cids)>0) {
			foreach ($cids as $cid) {
				...
				$feed->set_parameters('ga:year,ga:month', 'ga:visits', 1000, 'ga:year');
				$feed->init();
				if($feed->error()) {
					JError::raiseWarning( 500, JText::_('IMPORT_VIEW_MODEL_FEED_ERROR').' '.$feed->error());
				}
				...
				foreach ($data as $item){
					if($item->get_metric('ga:visits') > 0){
						...
						$endDate = strtotime('+1 month', $feed->get_start_date());
						if($endDate > time())
						$endDate = time();
						$feed->set_end_date($endDate);
						$feed->set_parameters('ga:date', 'ga:visits', 100, 'ga:date');
						$feed->init();
						$data = $feed->get_items();
						break;
					}
