...
class plgContentDioneMagicCalendar extends JPlugin
{
	function onPrepareContent( &$article, &$params, $page=0)
	{
		...
		if ( $count ) {
			...
			$lang_parameter     			= $this->params->get('lang', 'en');
			$showOn_parameter     			= $this->params->get('showOn', 'focus');
			$appendText_parameter     		= $this->params->get('appendText', '');
			$buttonText_parameter     		= $this->params->get('buttonText', 'Button text');
			$buttonImage_parameter     		= $this->params->get('buttonImage', '');
			$buttonImageOnly_parameter  	= $this->params->get('buttonImageOnly', 0);
			$changeMonth_parameter     		= $this->params->get('changeMonth', 0);
			$changeYear_parameter     		= $this->params->get('changeYear', 0);
			$yearRange_parameter     		= $this->params->get('yearRange', 'c-10:c+10');
			$showOtherMonths_parameter  	= $this->params->get('showOtherMonths', 0);
			$selectOtherMonths_parameter   	= $this->params->get('selectOtherMonths', 0);
			$showWeek_parameter     		= $this->params->get('showWeek', 0);
			$shortYearCutoff_parameter     	= $this->params->get('shortYearCutoff', '+10');
			$numberOfMonths_parameter     	= $this->params->get('numberOfMonths', 1);
			$showCurrentAtPos_parameter     = $this->params->get('showCurrentAtPos', 0);
			$stepMonths_parameter     		= $this->params->get('stepMonths', 1);
			$stepBigMonths_parameter    	= $this->params->get('stepBigMonths', 12);
			$autoSize_parameter     		= $this->params->get('autoSize', 0);
